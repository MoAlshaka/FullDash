<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
    ...
    <ul class="menu-inner py-1">
        ...
        @can('Admins & Roles')
            <li
                class="menu-item  {{ request()->is('*admin/roles*') ? 'open' : '' }} {{ request()->is('*admin/admins*') ? 'open' : '' }}">
                <a href="javascript:void(0);" class="menu-link menu-toggle">
                    <i class="menu-icon tf-icons ti ti-shield-half"></i>
                    <div> {{ __('sidebar.Admins&Roles') }}</div>
                </a>
                <ul class="menu-sub">
                    @can('Admins')
                        <li class="menu-item {{ request()->is('*admin/admins*') ? 'active' : '' }}">
                            <a href="{{ route('admins.index') }}" class="menu-link">
                                <div> {{ __('sidebar.Admins') }}</div>
